...
function location() {
	...
	console.log(result)
	config.setItem('location',result)
	console.log(result)
	return result
}
...
async function requestIOS(permissionID) {
	...
	return new Promise((resolve, reject) => {
		console.log(config.getItem(permissionID))
		if(config.getItem(permissionID)){
			console.log('我是')
			resolve(config.getItem(permissionID)*1 )
			return 
		}
		switch (permissionID) {
			case "push":
	...
	return new Promise((resolve, reject) => {
		...
		plus.android.requestPermissions(
			...
			function(resultObj) {
				...
				// 我来寸了
				config.setItem(permissionID,result+'')
				resolve(result)
			},
			function(error) {
